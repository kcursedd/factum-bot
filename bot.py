...
# =================== ВСТРОЕННАЯ БАЗА САЙТОВ SHERLOCK (355 сайтов) ===================
SITES_JSON = r"""
[
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🔍 *Factum OSINT Bot*\n"
        "Поиск по 355+ сайтам как в Sherlock, но лучше.\n"
        "Используй: `/factum никнейм`\n"
...
async def factum_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("⚠️ Укажи ник. Пример: `/factum target`", parse_mode="Markdown")
        return

    username = context.args[0].strip()
    msg = await update.message.reply_text(f"🔎 Ищу `{username}` по {len(sites)} сайтам...", parse_mode="Markdown")
    ...
    if not results:
        await msg.edit_text(f"❌ Ничего не найдено для `{username}`.\nПроверено {len(sites)} сайтов за {elapsed}с.", parse_mode="Markdown")
        return

    # Формируем красивый ответ
    found_count = len(results)
    response = f"🎯 *Factum — результаты для* `{username}`\n"
    response += f"📊 Найдено: {found_count} / {len(sites)} сайтов\n"
    response += f"⏱ Время: {elapsed}с\n\n"
    for site, url in results:
        response += f"✅ [{site}]({url})\n"

    # Если сообщение слишком длинное, обрезаем (но обычно влазит)
...
def main():
    ...
    logger.info("✅ Бот Factum запущен!")
    app.run_polling()
